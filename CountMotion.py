...
def main():
    ...
    if doplot:
        ...
        h1 = ax1.imshow(mot[0], origin="upper")
        ht1 = ax1.set_title("")
        # plot lanes
    ...
    for i, m in enumerate(bmot):
        # %% process each frame
        N = spatial_discrim(m, detect_min, detect_max, ax2)
        if i % frame_count_interval == 0:
            j += 1
            CarCount[j] = N
            if doplot:
                h3.set_ydata(np.cumsum(CarCount))
        # %% save plots
        if doplot:
            ht1.set_text(f"h.264 difference frames: index {i}, elapsed seconds {time[j]}")
            h1.set_data(mot[i])
            fg.canvas.draw()
            fg.canvas.flush_events()
            pause(0.001)
            if p.saveplot:
                fg.savefig(p.saveplot + f"{i:05d}.png", bbox_inches="tight", dpi=100)
...
def spatial_discrim(mot: np.ndarray, detect_min: float, detect_max: float, ax2=None) -> int:
    """
    ...
    # %% plot
    plotlanes(N1, N2, L, Flane1, Flane2, iLPF, ax2)

    return N1 + N2


def plotlanes(N1: int, N2: int, L: int, Flane1: np.ndarray, Flane2: np.ndarray, iLPF: typing.Tuple[int, int], ax2):
    if ax2 is None:
        return
